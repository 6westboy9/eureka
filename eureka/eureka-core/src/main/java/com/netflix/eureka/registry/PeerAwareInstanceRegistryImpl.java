...
public class PeerAwareInstanceRegistryImpl extends AbstractInstanceRegistry implements PeerAwareInstanceRegistry {
    ...
    // EurekaClient 接口实现类 DiscoveryClient
    ...
    public PeerAwareInstanceRegistryImpl(EurekaServerConfig serverConfig, EurekaClientConfig clientConfig, ServerCodecs serverCodecs, EurekaClient eurekaClient) {
        ...
        this.instanceStatusOverrideRule = new FirstMatchWinsCompositeRule(new DownOrStartingRule(), new OverrideExistsRule(overriddenInstanceStatusMap), new LeaseExistsRule());
    }
    ...
    public void init(PeerEurekaNodes peerEurekaNodes) throws Exception {
        ...
        // 初始化接口请求的缓存，走的是三层缓存机制
        initializedResponseCache();

        // 定时任务更新心跳次数（用于服务剔除与自我保护机制）
    ...
    @Override
    public int syncUp() {
        // 从对等 eureka 节点填充注册表信息。如果通信失败，此操作将失败转移到其他节点，直到列表耗尽为止。
        // Copy entire entry from neighboring DS node
        int count = 0;

        // 意思就是 count 为 0 时，会进行重试到最大重试次数，顺利的话只需要遍历一次
    ...
    public boolean isLeaseExpirationEnabled() {
        // Eureka Server 是否开启自我保护机制，如果没有开启，直接返回 true
        if (!isSelfPreservationModeEnabled()) {
            // The self preservation mode is disabled, hence allowing the instances to expire.
            return true;
        }
        // 每分钟接收心跳次数的阈值 > 0 并且 最近一分钟的心跳次数 > 每分钟接收心跳次数的阈值，返回 true
    ...
    private void updateRenewalThreshold() {
        try {
            ...
            synchronized (lock) {
                ...
                // 已经注册的服务实例数 > 配置的期望发送心跳的服务实例数量 * 85% || 自我保护机制关闭
    ...
    private void replicateToPeers(Action action, String appName, String id,
        ...
        try {
            ...
            for (final PeerEurekaNode node : peerEurekaNodes.getPeerEurekaNodes()) {
                ...
                // 重点
                // 同步注册表信息至其他 Eureka Server 节点，采用的是三层队列任务处理机制
     ...
     *
     * 当自己（Eureka Server）发生下线、心跳续约、注册等等时，自己本地的注册表信息肯定已经发生了变化，此时需要将其同步到 Eureka Server 集群中的其他节点
     *
     */
